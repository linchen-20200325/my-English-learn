...
- **冷熱資料分離**：專案根目錄必須維持極簡 `STATE.md`（含進度、Bug、檔案簡介）。每次任務**僅限讀取此檔與目錄結構**來理解專案目標，嚴禁要求使用者重複解釋。
...
- **跳 PR 直推例外**：以下情境可直接 push 至 `main`（執行 `./scripts/quick_merge.sh "msg"`），免走 PR：
  1) `STATE.md` / `CLAUDE.md` / 註解 / typo 修正
  2) 版本字串 bump（不含程式邏輯變更）
  3) 不影響功能行為的純文件改動（README、docs/）
  其他一律走 PR，保留 CI gate 與變更紀錄。

## §5 卡關救援 (Anti-Loop Protocol)
- 針對同一個報錯，若連續重試 2 次未果，**立即停機**。
- 輸出「外部 AI 諮詢清單」：精煉列出問題核心、錯誤 Log 與相關代碼片段，交由我詢問其他 AI 進行雙重驗證。
