...
def main() -> None:
    ...
    for cat_key, items_now in (("pre", PREFIXES), ("root", ROOTS),
                                ("suf", SUFFIXES)):
        new_items, added, unmatched = merge_into(items_now, bank.get(cat_key, {}))
        totals[cat_key] = added
        unmatched_all[cat_key] = unmatched
        new_blocks[cat_key] = render_block(CATEGORY_MAP[cat_key], new_items)

    print("=== 合併報告 ===")
    for k in ("pre", "root", "suf"):
        print(f"  {CATEGORY_MAP[k]:8} +{totals[k]} 字"
              + (f" (未匹配 m: {unmatched_all[k]})" if unmatched_all[k] else ""))
    total = sum(totals.values())
    print(f"  合計新增:{total} 字")
