...
## 🆕 本次優化（科學學習 + 閱讀 + 字庫）
- **閱讀理解測驗**（`comprehension.py`）：5 篇互動閱讀皆加入選擇題,讀完即時批改+解析,分數計入今日統計（主動回憶 active recall）。
- **記憶科學監督**（`view_progress` 強化）：複習牌組的記憶強度分布（新卡/學習中/漸熟/已掌握）、熟練比例、今日待複習、**未來 7 天複習負擔預測圖**。
- **字庫擴充**：`vocab_bank.json` 50 → 78 字（補入 learn/study/read/write/love/important… 等高頻字,含諧音/KK/例句/用法）。
